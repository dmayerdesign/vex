...
export function setUpDevtoolsForAngular(
  devtoolsOptions = DEFAULT_DEVTOOLS_OPTIONS,
  ngZone: { run: (cb: () => any) => any }
): void {
  return setUpDevtools(devtoolsOptions, ngZone)
}
  ...
  function _setUpDevtools(): void {
    ...
    vex_result$
      ...
      .subscribe(([ result, globalState ]) => {
        console.log('got vex result!', result, globalState)
    ...
    vex_dispatch$
      ...
      .subscribe(([{ actionType, lookupKey }, globalState]) => {
        console.log('got vex dispatch!', actionType, lookupKey, globalState)
    ...
    devtools.subscribe(({ type, state }) => {
      if (!!state) {
        ...
        console.log('[devtools]', type, globalState)
        ...
        if (type === 'DISPATCH') {
          vex_managers$.pipe(first()).subscribe((managers) => {
            ...
            console.log('TOP LEVEL', topLevelState)
            managers.forEach((manager) => {
              const lookupKey = manager.getLookupKey()
              console.log('[map]', mapGlobalStateToNested(manager.getLookupKey(), globalState))
              if (lookupKey === VEX_ROOT) {
